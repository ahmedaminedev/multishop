                            ...
                            >
                                <a 
                                    ...
                                    className={`
                                        ...
                                        ${hoveredCategory === category.name ? 'text-lime-600 dark:text-brand-neon' : 'text-gray-600 dark:text-gray-300 hover:text-lime-600 dark:hover:text-brand-neon'}
                                    `}
                                >
                                    {category.name}
                                    {hasChildren && (
                                ...
                                {(hoveredCategory === category.name && hasChildren) && (
                                    <div className="absolute left-0 top-full w-full bg-white dark:bg-gray-900 shadow-xl border-t border-brand-neon/30 animate-fadeIn min-h-[200px] z-50">
                                        <div className="max-w-screen-2xl mx-auto px-8 py-8">
                                            {hasMegaMenu ? (
                                                <div className="grid grid-cols-4 gap-8">
                                                                ...
                                                                {group.items.map((item, i) => (
                                                                    <li key={i}>
                                                                        <a 
                                                                            ...
                                                                            className="text-sm text-gray-500 dark:text-gray-400 hover:text-lime-600 dark:hover:text-brand-neon transition-colors block py-0.5 hover:translate-x-1 transform duration-200"
                                                                        >
                                                                            {item.name}
                                                                        </a>
                                                                    </li>
                                                    ...
                                                    {/* Image promo optionnelle dans le menu */}
                                                    <div className="col-span-1 bg-brand-neon/5 dark:bg-gray-800 rounded-lg p-6 flex flex-col justify-center items-center text-center border border-brand-neon/20 dark:border-gray-700">
                                                        <p className="font-serif text-lime-600 dark:text-brand-neon font-bold text-xl mb-2">Nouveautés</p>
                                                        <p className="text-sm text-gray-500 mb-4">Découvrez la collection {category.name}</p>
                                                        <button 
                                                            onClick={() => onCategoryClick(category.name)}
                                                            className="text-xs font-bold uppercase underline hover:text-lime-700 dark:hover:text-white"
                                                        >
                                                            Voir tout
                                                        </button>
                                                    </div>
                                                        ...
                                                        {category.subCategories!.map((sub, i) => (
                                                            <li key={i}>
                                                                <a 
                                                                    ...
                                                                    className="text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-lime-600 dark:hover:text-brand-neon transition-colors flex items-center gap-2 group"
                                                                >
                                                                    <span className="w-1.5 h-1.5 bg-gray-300 rounded-full group-hover:bg-brand-neon transition-colors"></span>
                                                                    {sub}
                                                                </a>
                                                            </li>
                                                        ))}
