...
// Fixed: ChevronDoubleLeftIcon does not exist in IconComponents, using ChevronLeftIcon instead
import { ListBulletIcon, ChevronRightIcon, ChevronLeftIcon, ChevronDownIcon } from './IconComponents';
    ...
    return (
        <nav ref={navRef} className="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 sticky top-20 lg:top-24 z-30">
            <div
                ...
                className="bg-brand-neon text-black font-bold text-base lg:text-lg p-3 flex items-center justify-between rounded-t-lg cursor-pointer select-none"
            >
                ...
                {/* Fixed: ChevronDoubleLeftIcon does not exist in IconComponents, using ChevronLeftIcon instead */}
                <ChevronLeftIcon className={`w-5 h-5 shrink-0 transition-transform duration-300 ${isCollapsed ? 'rotate-180' : ''}`} />
            </div>
                ...
                <ul className="py-2 max-h-[60vh] lg:max-h-none overflow-y-auto">
                    {categories.map((category) => (
                        <li key={category.name} className="relative border-b last:border-b-0 border-gray-100 dark:border-gray-700/50">
                            ...
                            {category.subCategories && !category.megaMenu && (
                                ...
                                `}>
                                    {category.subCategories.map((sub, i) => (
                                        <a key={i} href="#" onClick={(e) => { e.preventDefault(); onCategoryClick(sub); }} className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 pl-8 lg:pl-4 border-l-2 border-transparent hover:border-brand-neon lg:border-l-0">{sub}</a>
                                    ))}
                                </div>
                            )}
                            ...
                            {category.megaMenu && (
                                <div className={`
                                    lg:absolute lg:left-full lg:-top-px lg:w-[800px] lg:max-h-[80vh] lg:overflow-y-auto lg:bg-white lg:dark:bg-gray-800 lg:shadow-lg lg:rounded-r-md lg:border lg:border-gray-200 lg:dark:border-gray-700 lg:border-t-4 lg:border-brand-neon lg:z-30
                                    ${activeCategory === category.name ? 'block' : 'hidden lg:invisible lg:opacity-0'}
                                    transition-all duration-200 bg-gray-50 dark:bg-gray-900/50
                                `}>
                                            ...
                                            <div key={group.title} className="flex flex-col space-y-2 py-1">
                                                ...
                                                <ul className="space-y-1 lg:space-y-2 pl-2 border-l border-gray-300 dark:border-gray-600 lg:border-none lg:pl-0">
                                                    {group.items.map((item, i) => (
                                                        <li key={i}>
                                                            <a href="#" onClick={(e) => { e.preventDefault(); onCategoryClick(item.name); }} className="block text-sm lg:text-base text-gray-600 dark:text-gray-400 hover:text-lime-600 dark:hover:text-brand-neon">{item.name}</a>
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
