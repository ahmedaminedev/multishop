...
import type { User, Product, Category, Pack, Order, CartItem, CustomerInfo, ContactMessage, Advertisements, Brand, Store } from './types';
...
const PageLoader = () => (
    <div className="flex items-center justify-center min-h-[60vh] bg-white dark:bg-brand-dark">
...
const AppContent: React.FC = () => {
    ...
    const { clearCart, identifyUser } = useCart(); 
    ...
    useEffect(() => {
        const handleHashChange = () => {
            ...
            if (hash.includes('accessToken=') || hash.includes('reset-password') || hash.includes('success=') || params.get('payment')) return;

            if (path === '' || path === 'home') { setCurrentPage('home'); window.scrollTo(0, 0); }
                ...
                setSelectedCategory(catParam ? decodeURIComponent(catParam) : 'product-list');
                setCurrentPage('product-list');
                window.scrollTo(0, 0);
            } else if (path.startsWith('product/')) {
                const id = path.split('/')[1];
                if (id) { setSelectedProductId(Number(id)); setCurrentPage('product-detail'); window.scrollTo(0, 0); }
            } else if (path === 'promotions') { setCurrentPage('promotions'); window.scrollTo(0, 0); }
            else if (path === 'packs') { setCurrentPage('packs'); window.scrollTo(0, 0); }
            ...
            else if (path === 'admin') user?.role === 'ADMIN' ? setCurrentPage('admin') : window.location.hash = '#/';
        };
    ...
    useEffect(() => {
        const checkUrlParams = async () => {
            ...
            if (hash.includes('accessToken=')) {
                ...
                if (token) { setResetToken(token); setCurrentPage('reset-password'); }
            } else if (params.get('payment') === 'success') {
                const orderId = params.get('orderId');
                clearCart();
                addToast("Votre rituel est confirmé.", "success");
                if(user) api.getMyOrders().then(setOrders);
                if (orderId) { setSelectedOrderId(orderId); setCurrentPage('order-detail'); }
                else setCurrentPage('order-history');
        ...
        const loadData = async () => {
            try {
                const [productsData, packsData, categoriesData, storesData, adsData, brandsData] = await Promise.all([
                    api.getProducts(), api.getPacks(), api.getCategories(), api.getStores(), api.getAdvertisements(), api.getBrands() 
                ]);
                setProducts(productsData); setPacks(packsData); setCategories(categoriesData); setStores(storesData); setBrands(brandsData);
    ...
    useEffect(() => {
        const loadUserData = async () => {
            if (user) {
                try {
                    if (user.role === 'ADMIN') {
                        const [allOrders, allMessages, allPromos] = await Promise.all([api.getAllOrders(), api.getMessages(), api.getPromotions()]);
                        setOrders(allOrders); setAdminMessages(allMessages); setPromotionsData(allPromos);
                    } else {
                        const myOrders = await api.getMyOrders();
                        setOrders(myOrders);
                    }
                } catch (err) { console.error("User data error", err); }
            } else { setOrders([]); setAdminMessages([]); }
        };
        loadUserData();
    }, [user]);
    ...
    const handleOrderComplete = async (cartItems: CartItem[], customerInfo: CustomerInfo, paymentId?: string) => {
        const newOrder = {
            ...
            shippingAddress: { type: 'Domicile', street: customerInfo.address, city: customerInfo.city, postalCode: customerInfo.postalCode, isDefault: true },
        ...
        try {
            ...
            addToast("Protocole validé avec succès !", "success");
    ...
    const renderPage = () => {
            ...
            if (user?.role === 'ADMIN') {
                return (
                    <Suspense fallback={<PageLoader />}>
                        <AdminPage 
                            onNavigateHome={navigateToHome} onLogout={handleLogout}
        ...
        return (
            <div className="flex flex-col min-h-screen w-full transition-colors duration-300 bg-brand-bg dark:bg-brand-dark text-gray-800 dark:text-gray-100 font-sans">
                ...
                <Header 
                    user={user} onNavigateToLogin={navigateToLogin} isLoggedIn={!!user} onLogout={handleLogout}
                    onNavigateToFavorites={navigateToFavorites} onNavigateToProfile={navigateToProfile}
                    onNavigateToOrderHistory={navigateToOrderHistory} allProducts={products} allPacks={packs}
                    allCategories={categories} onNavigateToCategory={navigateToCategory}
                    onNavigateToProductDetail={navigateToProductDetail} onNavigateToCompare={navigateToCompare}
                />
                <NavBar 
                    onNavigateHome={navigateToHome} onNavigateToPacks={navigateToPacks}
                    onNavigateToPromotions={navigateToPromotions} onNavigateToBlog={navigateToBlog}
                    onNavigateToNews={() => {}} onNavigateToContact={navigateToContact}
                />
                <CategoryBar categories={categories} onCategoryClick={navigateToCategory} />
                <main className="flex-grow">
                    <Suspense fallback={<PageLoader />}>
                        {currentPage === 'home' && <HomePage onNavigate={navigateToCategory} onPreview={setPreviewProduct} onNavigateToPacks={navigateToPacks} products={products} packs={packs} advertisements={advertisements} onNavigateToProductDetail={navigateToProductDetail} categories={categories} brands={brands} />}
                        {currentPage === 'product-list' && <ProductListPage categoryName={selectedCategory} onNavigateHome={navigateToHome} onNavigateToCategory={navigateToCategory} onPreview={setPreviewProduct} onNavigateToPacks={navigateToPacks} products={products} onNavigateToProductDetail={navigateToProductDetail} categories={categories} activeFilters={activeFilters} />}
                        {currentPage === 'product-detail' && selectedProductId && (() => { const p = products.find(prod => prod.id === selectedProductId); return p ? <ProductDetailPage product={p} allProducts={products} onNavigateHome={navigateToHome} onNavigateToProductDetail={navigateToProductDetail} onPreview={setPreviewProduct} /> : <PageLoader />; })()}
                        {currentPage === 'packs' && <PacksPage onNavigateHome={navigateToHome} onNavigateToCategory={navigateToCategory} packs={packs} allProducts={products} allPacks={packs} onNavigateToPacks={navigateToPacks} onNavigateToPackDetail={navigateToPackDetail} categories={categories} />}
                        {currentPage === 'pack-detail' && selectedPackId && <PackDetailPage pack={packs.find(p => p.id === selectedPackId) || packs[0]} allProducts={products} allPacks={packs} onNavigateHome={navigateToHome} onNavigateToProductDetail={navigateToProductDetail} onNavigateToPackDetail={navigateToPackDetail} onNavigateToPacks={navigateToPacks} />}
                        {currentPage === 'login' && <LoginPage onNavigateHome={navigateToHome} onLoginSuccess={handleLoginSuccess} />}
                        {currentPage === 'profile' && user && <ProfilePage user={user} onNavigateHome={navigateToHome} onUpdateUser={(u) => setUser(u)} />}
                        {currentPage === 'contact' && <ContactPage onNavigateHome={navigateToHome} stores={stores} />}
                        {currentPage === 'promotions' && <PromotionsPage onNavigateHome={navigateToHome} onNavigateToCategory={navigateToCategory} onPreview={setPreviewProduct} products={products} onNavigateToProductDetail={navigateToProductDetail} />}
                        {currentPage === 'blog' && <BlogPage onNavigateHome={navigateToHome} onSelectPost={navigateToBlogPost} />}
                        {currentPage === 'blog-post' && selectedBlogPostSlug && <BlogPostPage slug={selectedBlogPostSlug} onNavigateHome={navigateToHome} onNavigateToBlog={navigateToBlog} />}
                        ...
                        {currentPage === 'reset-password' && <ResetPasswordPage onNavigateHome={navigateToHome} token={resetToken || ''} />}
                    </Suspense>
                </main>
                <Footer onNavigateToPrivacy={navigateToPrivacyPolicy} onNavigateToDataDeletion={navigateToDataDeletion} />
                {(!user || user.role !== 'ADMIN') && <SupportWidget user={user} />}
                <ScrollToTopButton />
                <CartSidebar isLoggedIn={!!user} onNavigateToCheckout={navigateToCheckout} onNavigateToLogin={navigateToLogin} />
